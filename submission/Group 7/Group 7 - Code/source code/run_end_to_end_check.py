...
sys.path.insert(0, str(PROJECT_ROOT / "source code" / "custom_decision_tree"))

from decision_tree_lab2.config import RANDOM_STATE, TEST_SIZE
from decision_tree_lab2.split import split_train_test


RESULTS_DIR = PROJECT_ROOT / "generated_results" / "validation"


def load_letter() -> dict[str, object]:
    train_path = PROJECT_ROOT / "dataset" / "letter_recognition" / "train.csv"
    test_path = PROJECT_ROOT / "dataset" / "letter_recognition" / "test.csv"
    ...
    return {
        ...
        "source": "dataset/letter_recognition/{train,test}.csv",
...
def main() -> None:
    ...
    datasets = [
        ...
        load_csv_dataset(
            "handwritten_digits",
            "target",
            PROJECT_ROOT / "dataset" / "handwritten_digits" / "digits.csv",
        ),
        load_csv_dataset(
            "covertype",
            "cover_type",
            PROJECT_ROOT / "dataset" / "covertype" / "covertype.csv",
        ),
    ]
