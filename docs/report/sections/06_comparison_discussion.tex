...
Nhóm dùng Bảng~\ref{tab:robustness-e4} để so sánh cùng một cấu hình kết hợp E4 với baseline trên ba bộ dữ liệu. Letter có $\alpha=0$ nên E4 tương đương HS trên cây chưa prune; Digits chọn $\lambda=0$ sau CCP; Covertype dùng cả $\alpha>0$ và $\lambda=10$.
...
Decision Tree là mô hình chính trong bài. Nhóm dùng Random Forest, SVM--RBF và KNN làm các reference baselines đại diện cho ensemble tree, kernel và instance-based learning \cite{breiman2001rf,cortes1995svm,cover1967knn}. Bảng~\ref{tab:best-models} tóm tắt mô hình tốt nhất trên từng dataset.
